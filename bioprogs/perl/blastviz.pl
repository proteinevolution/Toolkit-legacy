...
# check arguments
if((scalar @ARGV)!=4){
    print "Usage: blastviz.pl BLASTRESFILE ID BASEDIR IMG_URL [max_seqs (default:100)]\n";
    exit(1);
}else{
    $resfile = $ARGV[0];
    $resfile=~/^\S+\.(\S+)$/;
...
<script type='text/javascript'>domain_slider_show($qlen, $domain_start, $domain_end);</script>";
