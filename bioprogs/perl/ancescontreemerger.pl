#! /usr/bin/perl -w
#
# Ancescon Tree Merger - Utility Class to extract all data from an HHR file 
# Usage  : ancescontreemerger.pl -n <names File> -t <treefile> [-v] <int> [-nl] <int>
# Author : Joern Marialke 2011 Genecenter Munich


use strict;

my $usage = "Merge Ancescon Tool Data
Usage  : ancescontreemerger.pl -n <names File> -t <treefile> 
...
sub load_names(){
	...
	while ($line=<INFILE>) {
		#Init loop variables, only alive in on cycle
		my $real_name;		
...
sub replace_names(){
	...
	foreach my $key ( @nameSequence_keys ){ 
	 	...
	 	if($treedata=~ /$key:/ ){
	 		$treedata=~ s/$key:/$nameSequenceMap{$key}:/;
