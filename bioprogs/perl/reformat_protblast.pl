...
sub read_fas_a2m_a3m{
    ...
    # Moved Validation from the inner Section to the end of the file reader
    if (($unnamed_seq == 0) && ($n > 0)) {
	                print("\nERROR: $infile is not correct FASTA format!\n");
			exit(1);
	}
	# Single unnamed Sequence
	if (($unnamed_seq == 0) && ($n ==0)) {
		$names[$n]="Unnamed Input Sequence";
	}
...
sub read_clu{
    ...
    while($line = <INFILE>){
	$line_no++;
	if ($line =~/^\s*$/  && $flag==0) {next;}      #leerzeilen am anfang des files �berspringen
	if ($line =~/^clustal(.*)/i && $flag==0) {     #erste nicht leere zeile muss mit clustal anfangen
	    #print("\nfile: OK (CLUSTAL)\n");
	    $flag=1;
	}
	elsif($flag==1){	    	    
