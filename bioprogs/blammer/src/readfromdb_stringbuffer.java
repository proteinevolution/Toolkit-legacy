 ...
 * @version
 */
public class readfromdb_stringbuffer {
    ...
    //public readfromdb(File[] dbarr,HashMap tax,taxid mytaxid,float cpu,boolean checktax,boolean verbose) {
    public readfromdb_stringbuffer(File[] dbarr, float cpu, boolean verbose) {
        this.dbarr=dbarr;
        //this.tax=tax;
    ...
    boolean extractseqs(File[] infiles,String extending){
        ...
        for(int i=0;i<cpu;i++){
            myreads[i]=new readthread(seqnames,this);//tax,mytaxid,this);
        }// end for i
        int findex=0;
        for(int i=0;i<nofiles;i++){
            ...
            myreads[currfree]=new readthread(seqnames,this);//tax,mytaxid,this);
        ...
        for(int i=0;i<cpu-1;i++){
            mygets[i]=new getseqs(linesvec,seqnames,this);//tax,mytaxid,seqnames,this);
            ...
            while(killthread==false){//do until I say stop
                while(linevec.size()>0){
                    ...
                    if(readnew){//if I have read in some new data from the input vector
                        ...
                        //now check each substring that has at least one pipe symbol
        ...
        public void run(){
            ...
            try{
                ...
                for(int i=0;i<java.lang.reflect.Array.getLength(dbarr);i++){
                    seqbuff.setLength(0);//clear the sequence buffer
                    ...
                    while((inline=reader.readLine())!=null){
                        if(inline.startsWith(">")){//if this is the start of the next sequence
                            numberofentries++;
                            if(seqbuff.length()>0){//if this is not the first sequence
                                currarr[1]=seqbuff.toString();
                                while(outvec.size()>10000){//check to see that the other threads keep up with readhdd
                                    try{
                                        synchronized (this){
                                            //System.out.println(outvec.size());
                                            wait(10);
                                        }
                                    }catch (InterruptedException e){
                            ...
                            seqbuff.setLength(0);
                            currarr=new String[2];
                            currarr[0]=inline;
                            continue;
                        }// end if line begins with a >
                        seqbuff.append(inline);
                    }// end while !=null
                    if(seqbuff.length()>0){//do the same for the last element at the end of file
                        currarr[1]=seqbuff.toString();
                        synchronized (outvec){
                            outvec.addElement(currarr);
                        }// end synchronized
            ...
            try{
                ...
                while((inline=inread.readLine())!=null){
                    ...
                    if(inline.indexOf("Length =")>-1){
                         ...
                         */
                        //get the names from <a name=xxx></a><a href="xxx" > NAME </a> rest
                        String[] tmparr=namestr.split("<a href=\".*\"\\s*>",2);//split at first occurrance
                        if(java.lang.reflect.Array.getLength(tmparr)>1){
                            //System.out.println("Split to"+tmparr[1]);
                            tmparr=tmparr[1].split("</a>",2);
                            if(java.lang.reflect.Array.getLength(tmparr)>1){
                                //System.out.println("name="+tmparr[0]);
                                currname=tmparr[0].trim();
                            }
                        }
                        //}
