 ...
 * @version 
 */
public class b2malign_nopsy {

    /** Creates new b2malign */
    public b2malign_nopsy() {
    ...
    public String[] malign(BufferedReader inread,float coverage,double blastmax,double scval){
        ...
        try{
            while (((str=inread.readLine())!=null)&&(stopread==false)){
                ...
                if(str.indexOf("<a name = ")> -1){//if this substring exists
                    readname=true;
                    if(tmpvec.size()>0){
                ...
                if(readseq){
                    ...
                    if(str.indexOf("</PRE>")>-1){// if </PRE> is on the line
                        ...
                        if(((blastmax==-1)||(evalue<=blastmax))&&((scval==-1)||(scval<(score/subjectlength)))){//blastmax==-1 = don't check
