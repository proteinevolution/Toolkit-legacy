 ...
 * @version
 */
public class clualign {
    
    /** Creates new clualign */
    public clualign() {
    }
    
    public String[] align(String command, String[] inarr, int borderwidth, int verbose, int cpunum, String basename) {
        ...
        if(java.lang.reflect.Array.getLength(inarr)<(2*3)){//if I have less than 2 seqs
            if(verbose>0){
                System.out.println("-----"+((int)(java.lang.reflect.Array.getLength(inarr)/3))+" sequences, skipping clustal alignment.-----");
            }
            return inarr;
        }
        String cpustr=String.valueOf(cpunum);
        /*if(basename.indexOf(java.io.File.separator)>-1){
            int tmpint=basename.lastIndexOf(java.io.File.separator);
            String tmpstr1=basename.substring(0,tmpint+1);
            String tmpstr2=basename.substring(tmpint+1);
            tmpstr2=tmpstr2.replaceAll("\\W+","_");
            basename=tmpstr1+tmpstr2;
        }
         */
        File alnfile=new File(basename+"_tmp"+cpustr+".aln");
        File tmpfile=new File(basename+"_tmp"+cpustr+".txt");
        File dndfile=new File(basename+"_tmp"+cpustr+".dnd");
        ...
        for(int i=0;i<numberofseq;i++){
            ...
            for(int j=0;j<numberofblocks;j++){
                ...
                if((blockinfo[j][1]<=-1)||(blockinfo[j][1]>currseqlength)){
                    ...
                    seqaln[i][j]=inarr[(i*3)+2].substring(blockinfo[j][0]);//from blockinfo [j][0] to end of seq
                }// end if -1
            ...
            if(numberofblocks==0){
                System.out.println("0 blocks to align for seq "+i);
            }
            if(tolastpos){//if the last seqaln went to the end
                seqnoaln[i][numberofblocks]="";
            }
            if(tolastpos==false){
                if((verbose>0)&&(didprinttolast==false)){
                    didprinttolast=true;
                    if(verbose>2){
        ...
        try{//do some IO; write data to file, run clustal on file, read clustal outfile
            ...
            for(int i=0;i<numberofblocks;i++){
                ...
                PrintWriter outfile;
                try{
                    outfile=new PrintWriter(new BufferedWriter(new FileWriter(tmpfile)));
                }catch (IOException ex){
                    System.err.println("ERROR: unable to open "+tmpfile+" for writing!");
                    ex.printStackTrace();
                    return inarr;
                }
                int elemwrite=0;
                ...
                try{
                    ...
                    try{// wait for clustal to finish
                        ...
                        System.err.println("ERROR Interrupted Clustalw run");
                    }
                    //String currstr;
                    System.err.print(errout.toString());
                }catch (IOException ex2){
                    System.err.println("ERROR starting process "+command+" -INFILE="+tmpfile.getAbsolutePath());
                    ex2.printStackTrace();
                }
                //read clustal output
                //System.out.print(",");
    ...
    String[] convertaaseq(aaseq[] inarr, String[] innames){
        ...
        for(int i=0;i<numberofseq;i++){
            ...
            if(foundseq==false){
                System.err.println("ERROR found no sequence matching "+innames[i]);
    ...
    int[][] getblockinfo(String[] inarr,int borderwidth){
        int beginb=0;
        boolean begin=true;//if I find no conserved collumn I want to align all
        ...
        int colwidth=0;
        for(int pos=0;pos<maxseqlength;pos++){
            //loop through the sequences position by position (char by char)
            boolean nogap=true;
            ...
            if(nogap){//if this collumn has no gap
                colwidth+=1;
                if((colwidth>=borderwidth)&&(begin==true)){
                    ...
                    if(tmparr[1]-tmparr[0]>1){//check that I have more than one gap; otherwise no need to align
            ...
            if(nogap==false){//actually no need to check for that because of continue above
                if((colwidth>=borderwidth)&&(begin==false)){//if this is the first gap after a conserved collumn
                    beginb=pos;
                    begin=true;
                }
                colwidth=0;
