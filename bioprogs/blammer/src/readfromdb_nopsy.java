...
public class readfromdb_nopsy {
    ...
    public readfromdb_nopsy(File[] dbarr, HashMap tax, float cpu, boolean checktax, boolean verbose) {
        this.dbarr=dbarr;
        this.tax=tax;
        this.cpu=(int)cpu;
        ...
        public void run(){
            ...
            try{
                ...
                while((inline=inread.readLine())!=null){
                    ...
                    if(inline.indexOf("<a name =")>-1){//first for blast; second for psyblast output.
                        doname=true;
                    }
                    if(inline.indexOf("Length =")>-1){
                        ...
                        if(currname.equals("")==false){
                            ...
                            if(myseqnames.containsKey(currname)==false){
                                myseqnames.put(currname,new seqobj());
                            }
                        }// end if name = ""
                    }// end if length
                    if(doname){
                ...
                synchronized (seqnames){//now add the stuff in myseqnames to the seqnames hash
                    //for(int i=0;i<elements;i++){
                        //if(seqnames.containsKey(currname)==false){
                        //     seqnames.put(currname,myseqnames.get(currname));
                        //}
                    //}
