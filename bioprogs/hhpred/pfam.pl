...
#    &System("$blastpgp -b 0 -j 2 -h 0.001 -d $dbname -i $file -B $basename.psi -C $basename.chk > $basename.log 2>&1");
    &System("$blastpgp -a 2 -b 0 -j 1 -h 0.001 -d $dummydb -i $file -B $basename.psi -C $basename.chk > $basename.log 2>&1");
