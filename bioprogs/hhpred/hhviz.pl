...
while($line=<RESFILE>){
	...
	{
	    if ($3 >= $probcutoff || scalar(@hits) < $minhits) {
		my %tmp;
		my $m=$1;
		my $header = $2;
...
for(my $i=10;$i<=$matchcols;$i+=10) {
	...
	$im->filledRectangle($border+$i*$xscale-$linewidth/2,$border,$border+$i*$xscale+$linewidth/2,$border+$headheight+2*$marklen,$grey);
	$im->string(gdSmallFont,$border+$i*$xscale-$xoffset,$border+$headheight+2*$marklen,$i,$black);
    } else {
	$im->filledRectangle($border+$i*$xscale-$linewidth/2,$border,$border+$i*$xscale+$linewidth/2,$border+$headheight+$marklen,$grey);
    }
}
...
#    &drawBar($x1,$y1,$x2,$y2,$col,$leftopen,$rightopen);
    ...
    my ($red,$grn,$blu);
    #$mysignif;
    if ($hit{'prob'} > $probcutoff) {
      $signif = ($hit{'prob'}-$probcutoff)/(100-$probcutoff);
      my $col=4*$signif;
      if ($col>3) {
	  ...
	  #blue (0,0,1) -> grey(0.2,0.2,0.2) transition
	  $signif = ($hit{'prob'}/$probcutoff)**3;
	  $red = 0.2*(1-$signif);
      	  $grn = 0.2*(1-$signif);
    	  $blu = 0.2+0.8*$signif;
    }
