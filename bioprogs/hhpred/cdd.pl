...
###########################
# NEW
    &System("$hh/reformat.pl fas a3m $base.tmp $base.tmp -M first");
    if ($root=~/^COG/ || $root=~/^KOG/) {
	&System("cp $base.tmp $base.a3m");
    } else {
	&System("$hhblits/hhblits -i $base.tmp -o /dev/null -oa3m $base.a3m -n 1 -cpu 2 -mact 0.5");
    }
    &System("$hhblits/addss.pl $base.a3m");
###########################
