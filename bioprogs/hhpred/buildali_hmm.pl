...
my $Etail=100;       # Psiblast Evalue threshold for collecting set of homologs
my $Epsi=1E-5;       # Psiblast Evalue threshold for core alignment
my $E=1E-2;          # HMMer Evalue threshold for building output alignment
...
Options for building alignments:
 ...
 -e   <float> E-value for inclusion in PSI-BLAST profile (def=$E)
...
if ($options=~s/ -E\s+(\S+) //g) {$Etail=$1;}
if ($options=~s/ -epsi\s+(\S+) //g) {$Epsi=$1;}
...
if ($v>=3) {
    ...
    unlink( glob "$tmp.* $tmpdir/$root.core.*");
    rmdir( "$tmpdir");
}
if ($v>=2) {
...
#    $qseq=~tr/a-z//d; # remove 'x' symbols that signify domain insertions in scop/astral sequences
	...
	# Write DSSP and PSIPREd seqs to $base.pre.a3m
	if (!open (ALIFILE, ">$base.pre.a3m")) {warn ("ERROR: cannot open $base.pre.a3m: $!\n"); return 1;}
	for (my $i=0; $i<@nam; $i++) {print(ALIFILE "$nam[$i]\n$seq[$i]\n");}
	close(ALIFILE);
    }

    # Append alignment sequences to dssp- and psipred sequences
    &System("cat $tmp.a3m >> $base.pre.a3m ");
    &System("mv $base.pre.a3m $base.a3m ");
...
sub BuildAlignment() {
    ...
    my $corefile="$tmpdir/$root.core.psi"; # core alignment containing only safest homologs, in PSIBLAST-readable format 
    ...
    my $iter_hmm=0;                    # number of HMMer iterations done during current PSI-BLAST iteration
    my $iter_hmm_max=4;               # max number of HHMer searches per PSI-BLAST iteration
    ...
    my $nhits_psi_prev=-1;             # number of nhits from previous PSIBLAST search
    my $nhits_hmm_prev=-1;             # number of nhits from previous HMMer search
	...
	# Prepare $corefile
	# Search database for a set of homologs up to E-value $Etail
	&blastpgp("-e $Etail -d $db -i $seqfile","$blafile");
	# Extract core alignment from BLAST output and write to $corefile (in psi format)
	&System("$perl/alignhits.pl -v 2 -cov $cov0 $qid -e $Epsi $bcore -best -psi -q $seqfile $blafile $corefile");
	# Extract seed alignment from BLAST output and write to $a3mfile (in a3m format)
	&System("$perl/alignhits.pl -v $v2 -cov $cov0 $qid -e $Epsi $bcore -best -a3m -q $seqfile $blafile $a3mfile");
   ...
   # Iterate PSI-BLAST searches
	...
	if ($nseqin>1 || $iter>1) {
	    ...
	    # Search database for a set of homologs up to E-value $Etail
	    &blastpgp("-e $Etail -d $db -i $seqfile -B $psifile","$blafile");
 	}
	 
	# Extract set of homologs from BLAST output and write to $prefile
	$nhits_psi_prev=$nhits_psi; 
	$nhits_psi = &System("$perl/alignhits.pl -v $v2 -cov $cov0 $qid $sc $best -e $Etail $bopt -ufas $blafile $prefile");
	if ($v>=2) {printf ("Preselected %s sequences in PSI-BLAST round $iter (E-value=$Etail, db=%s)\n",&NHits($nhits_psi),$db_nopath);}
#	if ($nhits_psi_prev>$nhits_psi) {last;} # no more new PSI-BLAST hits found? => leave loop

	# Iterate HMMer searches
	$nhits_prev=$nhits; $nhits_hmm_prev=-1; $iter_hmm=0;
	while ($nhits<250 && $nhits_hmm_prev<$nhits && $iter_hmm<$iter_hmm_max) {
	    ...
	    # Calibrate HMM
	    &System("$hmmerdir/hmmcalibrate --cpu $cpu --num 1000 $hmmfile > /dev/null");
	    # Search set of homologs with HMM
	    &System("$hmmerdir/hmmsearch --cpu $cpu -Z ".&SizeDB($blafile)." $hmmfile $prefile > $hmrfile 2>&1");
#	    &System("$hmmerdir/hmmsearch --cpu $cpu $hmmfile $prefile > $hmrfile 2>&1");
	    # Extract alignment from HMMer results
	    &System("$perl/alignhmmer.pl -v $v2 -cov $cov0 -e $E -a3m -append -q $seqfile $hmrfile $a3mfile");
#	    exit;
	    # Filter for maximum sequence identity etc.
	    $nhits_hmm_prev=$nhits; 
	    $nhits=&System("$hh/hhfilter -v $v -M a3m -id $id $qid -cov $cov0 -diff $Ndiff -i $a3mfile -o $a3mfile ") || die();
	    $iter_hmm++;

 	    if ($v>=2) {printf ("Found %s sequences in hmmsearch round $iter:$iter_hmm (E-value=$E, db=%s))\n",&NHits($nhits),$db_nopath);}
	}
