...
use lib "/cluster/toolkit/production/bioprogs/hhpred";
...
sub ReadAlignment() {
	...
	$i=0;
        while ($line=~/^T\s+/) {
	    if ($line!~/^T\s+(ss_|sa_|aa_|Consens|Cons-|_cons)/ && $line=~/^T\s*(\S+)\s+(\d+)\s+(\S+)\s+(\d+)\s+\((\d+)/){
		#print "L944 $1 \n";
		$tname[$i]=$1;
...
# Beware: This function is different from the Three2One-Mapping i.e. of
# pdb2fasta.pl, because the mapping here has to be synchronized with modeller.
##################################################################################
sub Three2OneLetter {
    my $res=uc($_[0]);
