#! /usr/bin/perl -w
# Usage: hhsearchall.pl fileglob database.hhm [options]
# Run hhsearch for all globbed files in current directory 
...
if (scalar(@ARGV)<2) {
   print("
Run hhsearch for all globbed files 

and write *.hhr and *.scores files into current directory
Usage:   hhsearchall.pl fileglob database.hhm [hhsearch options] [options] 
Options:
...
Example: hhsearchall.pl '*.hhm' /home/soeding/scop50.2/db/scop50.hhm -u  > hhsearchall.log 2>&1 &
\n"); 
    exit;
}
...
for ($i=$first; $i<@files; $i++) {   
    ...
    if ($update==1 && -e "$rootname.hhr") {next;}
    print(">>>>>>>> $ifile
: $file <<<<<<<<\n");
    &System("$hh/hhsearch -i $file -d $db -o $rootname.hhr -scores $rootname.scores $options");
    print("\n");
}
print("Finished hhsearchall\n");
exit(0);
