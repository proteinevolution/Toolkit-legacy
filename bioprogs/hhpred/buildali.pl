...
my $msa="first";     # match state assignment
...
Options for building alignments:
 ...
 -cov  <int>   minimum coverage in % (Coverage = length of HSP / length of query) 
               (def=$cov)
 -M    first   make all columns with residue in first sequence match columns (default)
 -M    <int>   make all columns with less than X% gaps match columns (not used if input format is a3m)
...
if ($options=~s/ -M\s+(\d+) / /g)   {$msa=$1;}
if ($options=~s/ -M\s+first / /g)   {$msa="first";}
...
if ($olddir ne "" && -e "$olddir/$inroot.a3m") {
    ...
    if ($informat eq "fas") {
	&System("perl $perl/reformat.pl -M $msa -noss fas a3m $infile $tmp.in.a3m",$v2);
    } elsif ($informat eq "a2m") {
	&System("perl $perl/reformat.pl -M $msa -noss a2m a3m $infile $tmp.in.a3m",$v2);
    } elsif ($informat eq "clu") {
	&System("perl $perl/reformat.pl -M $msa -noss clu a3m $infile $tmp.in.a3m",$v2);
    } elsif ($informat eq "sto") {
	&System("perl $perl/reformat.pl -M $msa -noss sto a3m $infile $tmp.in.a3m",$v2);
    } else {
