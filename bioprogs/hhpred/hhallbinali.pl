#! /usr/bin/perl -w
# Read in all globbed *.scores files, bin results and write out a table cumulative or density counts
...
if (scalar(@ARGV)<2) {
    print("
 Read in all globbed *.scores files, bin results and write out a table cumulative counts
 SCORE   TOT  FAM  SFAM  FOLD
 which lists how many pairs where found at the family, superfamily and fold level 
 with Maxsub or S3D scores in each bin (-dens) or at or above the given value (-cum)
 Usage:   hhallbinali.pl [-ms|-comb|-s3d|-av] [-cum|-dens] fileglob outfile 
 Example: hhallbinali.pl -s3d '*.scores' scop20_cumms.dat   
\n"); 
    exit;
}
