#! /usr/bin/perl -w
#
# Generate a model from an output alignment of blast (or psiblast). 
# The model output format can be either AL (alignment) or TS (pdb-format).

# Usage: blastmakemodel.pl -i file.out (-ts file.pdb|-al file.al) [-m int] [-pdb pdbdir] 
BEGIN {
    push (@INC,"/home/soeding/perl"); 
    push (@INC,"/home/soeding/hh"); 
    push (@INC,"/cluster/user/soeding/perl");    # for cluster
    push (@INC,"/cluster/user/soeding/hh");      # for cluster
}
use strict;
...
my $usage="
 Generate a 3D model from an output alignment of blast. 
 The model output format can be either AL (alignment) or TS (pdb-format).
 Usage: $program [-i] file.hhr [options]
 Options:
  -i  file.bla      results file from blast with hit list and alignments
...
my $query;       # name of query from blast output file (infile)
my $template="";   # name of template (hit) from blast output file (infile)
my $pdbfile;     # name of pdbfile to read
my $pdbcode;     # four-letter pdb code in lower case and _A if chain A (e.g. 1h7w_A)
my $aaq;         # query amino acids from blast output
my @aaq;         # query amino acids from blast output
my $aat;         # template amino acids from blast output
my @aat;         # template amino acids from blast output
...
my $i=0;         # counts query  residues from blast output
my $j=0;         # counts template residues from blast output
my $l=1;         # counts template residues from pdb file (first=1, like for i[col2] and j[col2]
my $col1=0;      # counts columns from blast alignment
my $col2=0;      # counts columns from alignment (by function &AlignNW) of $aat versus $aapdb 
my @i1;          # $i1[$col1] = index of query  residue in column $col1 of blast-alignment
my @j1;          # $j1[$col1] = index of template residue in column $col1 of blast-alignment
my @j2;          # $j2[$col2] = index of blast template seq in $col2 of alignment against pdb template sequence
my @l2;          # $l2[$col2] = index of pdb template seq in $col2 of alignment against blast template sequence
...
while ($line=<INFILE>) {if ($line=~/^Query=\s*(\S+)/) {$query=$1; last;} }

# Find pickhit'th hit in blast file
$hit=0;   # counts hits in hit list
while ($line=<INFILE>) {
    if ($line=~/^>(\S+)/) {
	$hit++;
	if ($hit>=$pickhit) { $template=$1; last;}
    }
}
...
sub ReadAlignment() {
    ...
    # Search beginning of alignment
    while ($line=<INFILE>) {if ($line=~/^\s*Identities =/) {last;} }
    $line=<INFILE>;
	...
	if (!$i) {$i=$1;} # if $i==0 then this is the first query line read -> set $i to >0
	if (!$i) {$i=1;}  # if still $i==0 then set $i to 1
	$aaq.=$2;
	$line=<INFILE>;
	$line=<INFILE>;
	if ($line!~/^Sbjct:\s*(\d+)\s+(\S+)/) {
	    die ("Error in $program: incorrect format in $infile: $!\n");
	}		
