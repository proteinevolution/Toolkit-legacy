...
inline double logPvalue_HHblast(double s, double corr)
{
...
inline double Pvalue_HHblast(double s, double corr)
{
...
inline double logLikelihood_HHblast(double s, double corr)
{
...
/////////////////////////////////////////////////////////////////////////////////////
//// Evaluate the *negative* log likelihood for the order statistic of the uniform distribution
...
/////////////////////////////////////////////////////////////////////////////////////
double HitList::RankOrderFitCorr(double* v)
{  
  double sum=0.0;
//   printf("%8.2G  %8.2G  %i\n",v[0],v[1],Nprof);
...
double HitList::RankOrderFitCorr_static(void* pt2hitlist, double* v)
{
...
/////////////////////////////////////////////////////////////////////////////////////
//// Evaluate the *negative* log likelihood of the data at the vertex v = (corr,offset)
//// Needed to fit the correlation and score offset in HHblast
/////////////////////////////////////////////////////////////////////////////////////
double HitList::LogLikelihoodCorr(double* v)
{  
  double sum=0.0;
//   printf("%8.2G  %8.2G  %i\n",v[0],v[1],Nprof);
...
// ( see http://www.newty.de/fpt/callback.html#member )
double HitList::LogLikelihoodCorr_static(void* pt2hitlist, double* v)
{
  HitList* mySelf = (HitList*) pt2hitlist;  // explicitly cast to a pointer to Hitlist
...
/////////////////////////////////////////////////////////////////////////////////////
void HitList::CalculateHHblastCorrelation(HMM& q)
{
  int nfunc=0;                     // number of function calls
...
inline double corr_HHblast(float Nq, float Nt)
{
  return 0.5;
}

//// Calculate HHblast composite E-values 
inline double offset_HHblast(float Nq, float Nt)
{
  return 0.0;
}
...
void HitList::ReadBlastFile(HMM& q)
{
  ...
  while (fgetline(line,LINELEN-1,blaf) && strncmp(line,"Database:",9));
  if (! fgetline(line,LINELEN-1,blaf)) FormatError(par.blafile);
  char* ptr=line;
  Ndb = strint(ptr);
  if (Ndb==INT_MIN) FormatError(par.blafile);
  while ((i=strint(ptr))>INT_MIN) Ldb = 1000*Ldb + i;
  if (Ldb==0) FormatError(par.blafile);
  printf("Number of sequences in database = %i    Size of database = %i\n",Ndb,Ldb);
	  ...
	  if (!blast_logPvals->Contains(templ)) // store logPval only for best HSP with template 
	    {
	      ...
	      if (!ptr) FormatError(par.blafile);
	      if (sscanf(ptr+8,"%g",&Eval)<1) FormatError(par.blafile);
	      // Calculate P-value[seq-seq] = 1 -  exp(-E-value[seq-seq]) = 1 - exp(-Lt/Ldb*E-value[seq-db])
	      Pval = length/double(Ldb)*double(Eval);
	      if (Eval>1E-3) logPval = log(1.0-exp(-Pval)); else logPval=log(double(Pval)+1.0E-99); 
	      blast_logPvals->Add(templ,logPval);
// 	      printf("template=%-10.10s  length=%-3i  Evalue=%8.2g  P-value=%8.2g\n",templ,length,Eval,exp(logPval));
	    } 
	  else
	    delete[] templ;
	}
