...
for ($nseed=0; $nseed<scalar(@seed_in); $nseed++) {
    ...
    foreach $indir (@indirs) {
    	print "L230 $indir \n";
	if ($names[$nseed]=~ /(gi\|\d+)\|/) {
		print "L231   matches GI\n";
	    $alifile = $1;
	    $alifile =~ s/\|/_/g;
	    my @tmp = glob("$indir/$alifile"."*.a3m");
	    if (scalar(@tmp) > 0) {
		$alifile = $tmp[0];
		last;
	    } else {
	...
	} elsif($names[$nseed]=~ /(PF\d+)_cons/){
		
	    $alifile = $1;
	    
	    my @tmp = glob("$indir/$alifile"."*.a3m");
	    print "L246   matches PFAM   $indir/$alifile".".a3m \n";
	    if (scalar(@tmp) > 0) {
		$alifile = $tmp[0];
		 print "L248   matches PFAM Scalar  $alifile\n";
		last;
	    } else {
		$alifile = "";
	    }
	    ...
	    $alifile=$indir."/".$names[$nseed].".a3m";
	    print "L253   matches ELSE   $alifile \n";
	    my $alifilefas=$indir."/".$names[$nseed].".fas";
	    if (-e $alifile) {last;}
	    if (-e $alifilefas) {
		&System("perl $perl/reformat.pl -M first $alifilefas $alifile");
    ...
    print "L266 Alignment File : $alifile \n";
    if ($alifile eq "") {die("Error: could not find $names[$nseed] in input directory/-ies.\n");}
    if ($diff ne "") {
	&System("$hh/hhfilter -v $v -i $alifile -o $tmpfile -diff $diff -cov 5");
	$alifile=$tmpfile;
    } 
