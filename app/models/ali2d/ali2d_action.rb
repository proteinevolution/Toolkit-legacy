class Ali2dAction < Action
  ...
  def perform
    ...
    @commands << "#{JAVA_1_5_EXEC} -jar #{ALI2D}/prepareAli2d.jar #{@infile} #{@basename} #{@seqident} #{@mainlog} &> #{job.statuslog_path}"
  ...
  def build_params
    ...
    @commands << "#{JAVA_1_5_EXEC} -jar #{ALI2D}/buildParams.jar #{@infile} #{@mainlog} &> #{@outfile} "
