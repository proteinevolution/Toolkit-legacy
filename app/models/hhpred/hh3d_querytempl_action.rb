class Hh3dQuerytemplAction < Action
	...
	def before_perform_on_forward
		...
		#FileUtils.cp(parent_basename + ".templ.pdb", @basename + ".templ.pdb")
	...
	def sup3d
		...
		while (1) do
			...
			while (pos < lines.length && lines[pos] =~ /^Q\s+(ss_dssp|ss_pred|ss_conf|aa_pred|Consensus|Cons-)/ ) do
				pos += 1
			end
			...
			while (pos < lines.length && (lines[pos] !~ /^T\s+\S+/ || lines[pos] =~ /^T\s+(ss_|aa_|Consensus|Cons-)/)) do
				pos += 1
			end
