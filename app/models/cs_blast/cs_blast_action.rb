class CsBlastAction < Action
  ...
  def perform
    ...
    @commands << "#{CSBLAST}/csblast -i #{@infile} -D #{CSDB}/cluster.prf --blast-path #{BLAST} -e #{@expect} -F #{@filter} -M #{@mat_param} -G #{@gapopen} -E #{@gapext} -v #{@descriptions} -b #{@alignments} -T T -o #{@outfile} -d \"#{@db_path}\" -I T -a 1 #{@other_advanced} &>#{job.statuslog_path}"
