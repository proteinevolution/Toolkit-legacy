class DataaAction < Action
  ...
  # Put action initialisation code in here
	...
	def perform
		...
		@commands << "BIOPROGS_DIR=#{BIOPROGS} #{BAD}/do_search.pl -i #{@infile} -td #{job.job_dir} -id #{job.jobid} -lp '#{job.url_for_job_dir}' -fg #{@fragcheck} 1> #{@outfile} 2> #{job.statuslog_path}"
