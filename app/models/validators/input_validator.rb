        ...
        def validates_input (*attr_names)
          ...
          if (!configuration[:informat_field].nil?)
            if configuration[:max_seqs].nil?
              validates_fasta(attr_names, {:min_seqs => configuration[:min_seqs], :on => :create, :informat => configuration[:informat], :informat_field => configuration[:informat_field], :inputmode => configuration[:inputmode], :max_length => configuration[:max_length],:header_length => configuration[:header_length], :single_gaps => configuration[:single_gaps], :ss_allow => configuration[:ss_allow] })
...
rstuvwyz/,+&\\-", :message => "HHrepID does not allow unidentified amino acids", :max_length => configuration[:max_length] ,:header_length => configuration[:header_length], :ss_allow => configuration[:ss_allow]})                     
            ...
            when "shx"
             #validates_input_fields(attr_names[1], {:on => :create, :input_file => attr_names[1], :allow_nil => configuration[:allow_nil]})
              logger.debug "##### SHIFTX!"
