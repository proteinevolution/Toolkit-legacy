        ...
        def validates_a3m (*attr_names)
          ...
          validates_each(attr_names, configuration) do | record, attr, value |
            ...
            if (!configuration[:informat_field].nil?)
              ...
              if (!format.nil? && format != "a3m")
                next
              end
              if (format.nil? && !configuration[:informat].nil? && configuration[:informat] != "a3m")
                next
              end
            else
              if (!configuration[:informat].nil? && configuration[:informat] != "a3m")
                next
              end
            end
...
ActiveRecord::Base.class_eval do
  include Toolkit::Validations::ValidatesA3M
end
